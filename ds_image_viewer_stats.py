...
class ResourceMonitor:
    ...
    def display_statistics(self):
        ...
        def export_stats():
            ...
            export_file = f"H:\\FLET\\resource_stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            try:
        ...
        close_button = ttk.Button(export_frame, text="Close", command=stats_window.destroy)
...
class ImageViewer:
    ...
    def on_closing(self):
        """Handle window closing event - display resource statistics first."""
        # Stop monitoring the current image if any
        if self.current_image_path:
            self.resource_monitor.stop_monitoring()
